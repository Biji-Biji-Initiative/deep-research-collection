...
touch collection/[repo-name]/deep-research/[topic-name]/result.md
```

### 5. Update Indexes
- Update `collection/[repo-name]/README.md` with research entry
- Update main `README.md` repository index

### 6. Commit and Push
```bash
git add collection/[repo-name]/
...
### List All Research
```bash
find collection/ -name "prompt.md" -type f
```

### Search Research Content
```bash
grep -r "search term" collection/ --include="*.md"
```
...
for dir in collection/*/; do
  ...
  echo "${dir#collection/}: $count research reports"
done
```
